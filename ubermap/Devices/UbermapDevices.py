...
class UbermapDevices:
    ...
    def get_device_name(self, device, override = False, latest_mode=False):
        if not device:
            return ''
    ...
    def get_device_filename(self, device, latest_mode=False):
        name = self.get_device_name(device, latest_mode=latest_mode)
    ...
    def dump_device(self, device):
        ...
        cfg = self.get_device_config(device)
        config = ConfigObj()

        if cfg and cfg.overriden:
            config.filename = self.get_device_filename(device, latest_mode=True)
        else:
    ...
    def get_device_config(self, device):
        cfg = config.load(self.get_device_name(device, override=True), 'Devices')
